...
private:
    GLuint BufferID = 0; // OpenGL assigned buffer ID
    int BufferType; // Buffer type (GL_ARRAY_BUFFER, GL_ELEMENT_BUFFER...)

    std::vector<unsigned char> RawData; // In-memory raw data buffer, used to gather the data for VBO.
    size_t BytesAdded = 0; // Number of bytes added to the buffer so far
    size_t UploadedDataSize; // Holds buffer data size after uploading to GPU

    bool IsBufferCreated = false; // Flag telling if the buffer has been created
    bool IsDataUploaded = false; // Flag telling, if data has been uploaded to GPU already.
};
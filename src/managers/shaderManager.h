...
namespace XE
{
    ...
    private:
        ...
        std::map<std::string, std::unique_ptr<Shader>> VertexShaderCache; // Vertex shader cache - stores vertex shaders within their keys in std::map
        std::map<std::string, std::unique_ptr<Shader>> FragmentShaderCache; // Fragment shader cache - stores fragment shaders within their keys in std::map
        std::map<std::string, std::unique_ptr<Shader>> GeometryShaderCache; // Gemetry shader cache - stores geometry shaders within their keys in std::map
    };
} // namespace XE